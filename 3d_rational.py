                            ...
                            for n1 in range(-L, (L+1)):
                                for n2 in range(-L, (L+1)):
                                    for n3 in range(-L, (L+1)):
                                        ...
                                        s2 = m2 + n2 - k2
                                        s3 = m3 + n3 - k3
                                        if np.abs(s1)<eps and np.abs(s2)<eps and np.abs(s3)<eps:
...
def main():
    ...
    print((end - start)/60)
    pd.DataFrame(counts).to_csv("RNS3dim_10.csv")

    ar = np.genfromtxt("RNS3dim_10.csv",delimiter=',')
    ar = ar[1:, :]
    ar = ar[:, 1:]
